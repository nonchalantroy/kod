...
const
   ...
   {1.01 has minor screen adjustments, and an adjustment of polity code.
    ...
    3.201 Bug fix, Indy components were not properly integrated, so user data download didn't work.  Fixed 10/29/07.
...
type
   ...
   one_user_data_set_selections = record
         ...
         data_set_variables : array of integer;  {has the original number of each variable}
      end;   {record}
   ...
   function Tuser_data_set_listing_obj.get_data_set_var_number (data_set_num : integer; var_name : string): integer;
      ...
      begin
         ...
         if get_data_set_var_name(data_set_num, curr_var) <> var_name then
            EUGeneError ('Error in generic set up vars - Could not find variable name in configuration information for variable name '+var_name+' when trying to find number.  Programming error - notify programmer', 3, stop, error_log)
