...
procedure TProgForm.BarInc (title : string; timeleft, pctdone : string);
  begin
    if (StrLen(Pchar(title)) > 30) then
      begin
        self.Height := 221;
        ProgFormLabel.Height := 34;
        ProgFormPanel.Height := 110;
        ProgressBar.Top := 51;
        TimeRemainingLabel.Top := 78;
        PctLeftLabel.Top := 78;
        StopExecution.Top := 146;
        PauseButton.Top := 146;
      end;
...
begin
    if (frame.global_initialized) then
       begin
          try
   ...
   {if main program not initialized, then do nothing, they can't stop yet!}
end;
