...
uses
  Cntry, Setting, OutWindow, PagedOutput, Abouteu, MenuHelp,
  ErrBx, YrBox, SysUtils, WinTypes, WinProcs, Classes, Graphics,
  Controls, Forms, Dialogs, Menus, StdCtrls, ExtCtrls, Buttons,
  CmnProcD, EUinoutD, EUtypes1, EUtypes2,
  EUTestD, GABox, RiskAndSecurityConversionProcs, riskgenbox,
  riskcombineintegrated, UserDataFileVars;
...
type
  TFrame = class(TForm)
    ...
    NonDirectedDyadYearOutputDataChoice: TMenuItem;
    ...
    Onset1: TMenuItem;
    Year1: TMenuItem;
    OutputEUProjectedJoiningInfo1: TMenuItem;
    ...
    procedure NonDirectedDyadYearOutputDataChoiceClick(Sender: TObject);
    ...
    procedure Onset1Click(Sender: TObject);
    procedure Year1Click(Sender: TObject);
    procedure OutputEUProjectedJoiningInfo1Click(Sender: TObject);
  private
...
uses Calcbox, euprocs1, EUProcs2, MemoUnit, RiskOverwriteForm,
  {UserDataConfigureForm, }FTPConnect,
  UserDataPreparationForm, ShellApi, Randy, MidOverTime, EUTypesMID, dyadgen, allianceoutput, browser;

{$R *.DFM}

{---------------------------------------------------------------}

procedure TFrame.SearchTopic1Click(Sender: TObject);
begin
...
procedure TFrame.AboutEugene1Click(Sender: TObject);
begin
     Abouteu.AboutBox.showmodal;
end;

{-------------------------------------------------------------}

procedure TFrame.FileMenuExitClick(Sender: TObject);
    begin
         Close;
    end;
...
procedure TFrame.CountryYearDataOutputMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
...
procedure TFrame.DirectedDyadYearDataOutputMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
      output_sub_call (ccode_index, nation_list, contiguity_data, configuration, user_selections,
...
procedure TFrame.NonDirectedDyadYearOutputDataChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
      output_sub_call (ccode_index, nation_list, contiguity_data, configuration, user_selections,
...
procedure TFrame.DisputeDyadDataOutputMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
      output_sub_call (ccode_index, nation_list, contiguity_data, configuration, user_selections,
...
procedure TFrame.PercentSystemCapabilitiesMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
...
procedure TFrame.TaubScoresUtilityMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
...
procedure TFrame.SMenuChoiceClick(Sender: TObject);
var option_result : integer;
begin
   try
...
procedure TFrame.ExpectedUtilityWarTrapMenuChoiceClick(Sender: TObject);
var option_result : integer;
    temp_name : TFileName;
begin
   try
         ...
         begin
            try
               ...
               case user_selections.similarity_method of
                 ...
                 else error ('Came into compute_and_save_EUWarTrap file check procedure without s/tau option set.  NOtify programmer.  Fatal error.  ',2,stop,error_log);
               end;   {case}
...
procedure TFrame.ExpectedUtilityWarandReasonMenuChoiceClick(Sender: TObject);
var option_result : integer;
   ...
   try
      ...
      case user_selections.similarity_method of
         ...
         use_s : case user_selections.s_weighting of
                  ...
                  else error ('Problem in mdiframe - sweighting not right value when calling compute EU war reason.  Fatal error - notify programmer.', 5, stop, error_log);
                 end;
         else error ('Problem in mdiframe - similarity_method not use_tau or use_s when calling compute EU war reason.  Fatal error - notify programmer.', 5, stop, error_log);
      end;  {case}
      {disable the buttons that would allow user to start procedures that might
       interfere with running procedures, then call procedure.  }
      if option_result = mrOK then
...
procedure TFrame.Contents1Click(Sender: TObject);
begin
   try
...
procedure TFrame.TraceOnClick(Sender: TObject);
begin
...
procedure TFrame.TraceOffClick(Sender: TObject);
begin
...
procedure TFrame.ShowOutputWindow1Click(Sender: TObject);
begin
   outputWindow.show;
end;
...
procedure TFrame.TestMenuClick(Sender: TObject);
begin
   testclick;
end;
...
procedure TFrame.RiskDataConvert1Click(Sender: TObject);
begin
   ShowMessage ('Risk Data has already been converted.  This procedure does not need to be run again.');
   {riskfileconvert; }
end;

procedure TFrame.RiskDataShow1Click(Sender: TObject);
begin
   risk_data_show;
end;

procedure TFrame.SecAllianceFileCreate1Click(Sender: TObject);
begin
...
procedure TFrame.RiskAllYears1Click(Sender: TObject);
         ...
         try
            ...
            case user_selections.similarity_method of
              ...
              use_s : case user_selections.s_weighting of
                       ...
                       else error ('Problem in mdiframe - sweighting not right value when calling compute Risk all yrs.  Fatal error - notify programmer.', 5, stop, error_log);
                    end;
              else error ('Came into All years risk proc from main menu procedure without s/tau option set.  NOtify programmer.  Fatal error.  ',2,stop,error_log);
            end;   {case}
            ...
            if (fileExists (temp_risk_file_name)) then
              begin
                 ...
                 if (filesize(risk_file)>0) then
                    begin
                       try
                          RiskOverwriteDeleteForm := TRiskOverwriteDeleteForm.create (frame);
...
procedure TFrame.RiskSubsetofYears1Click(Sender: TObject);
var RiskYearBox : TRiskGenForm;
    RiskOutFileName, temp_security_alliance_file_name : TFileName;
begin
   try
      user_selections.compute_this := compute_risk;
      RiskYearBox := TRiskGenForm.create(frame);
      {RiskYearBox.font:=frame.font;}
               ...
               try
                  ...
                  case RiskYearBox.OverwriteRiskRecords of
                     true: case user_selections.similarity_method of
                          ...
                          else error ('Came into risk subset year proc from main menu procedure without s/tau option set.  NOtify programmer.  Fatal error.  ',2,stop,error_log);
                        end;   {case}
...
procedure TFrame.RiskAppendNewYears1Click(Sender: TObject);
...
procedure TFrame.RiskSingleDisplayProcedureClick(Sender: TObject);
...
procedure TFrame.CombineRiskFilesMenuItemClick(Sender: TObject);
begin
   CombineRiskFileForm.showmodal;
end;

procedure TFrame.showsecuritytaumenuClick(Sender: TObject);
begin
   security_data_show (configuration.security_alliance_Tau_file_name);
end;

procedure TFrame.ShowSecuritySMenuClick(Sender: TObject);
begin
   security_data_show (configuration.security_alliance_S_unweighted_file_name);
end;

procedure TFrame.TransferUserDataSets1Click(Sender: TObject);
begin
...
procedure TFrame.CreateDataConfigClick(Sender: TObject);
begin
...
procedure TFrame.RunRandysProcedureClick(Sender: TObject);
begin
...
procedure TFrame.SaveSettingsMenuClick(Sender: TObject);
      ...
      begin
        ...
        {These sections are special because of pointers, indefinite arrays, etc.
         ...
         if saved_settings_for_input.user_selections.num_dyads = 0 then
            ...
            begin
               for counter := 0 to saved_settings_for_input.user_selections.num_dyads do
                  begin
                     new(current_user_dyad);
                     current_user_dyad^.ccode1 := saved_settings_for_input.user_selections.user_specified_dyads[counter].ccode1;
                     current_user_dyad^.ccode2 := saved_settings_for_input.user_selections.user_specified_dyads[counter].ccode2;
                     current_user_dyad^.styear := saved_settings_for_input.user_selections.user_specified_dyads[counter].styear;
                     current_user_dyad^.endyear := saved_settings_for_input.user_selections.user_specified_dyads[counter].endyear;
         ...
         for counter := 0 to saved_settings_for_input.user_selections.num_user_data_sets-1 do
            begin
               {first count how many legit variables}
               varcount := 0;
               for counter2 := 0 to max_user_variables_per_data_set do if
                  saved_settings_for_input.user_selections.user_data_sets_selections[counter].data_set_variables[counter2] <> -1 then
                  inc(varcount);
               setlength (user_data_sets_selections[counter].data_set_variables, varcount);
               {Also need to put user var settings into the form}
               setLength(Saved_Var_Info[counter].data_set_variables, varcount);
               {now add variables}
               for counter2 := 0 to varcount-1 do
                  begin
                     user_data_sets_selections[counter].data_set_variables[counter2] :=
                        saved_settings_for_input.user_selections.user_data_sets_selections[counter].data_set_variables[counter2];
                     Saved_Var_Info[counter].data_set_variables[counter2] := saved_settings_for_input.user_selections.user_data_sets_selections[counter].data_set_variables[counter2];
                  end;
            end;
...
procedure TFrame.ViewDocumentation1Click(Sender: TObject);
begin
    ShellExecute(Handle, 'open',
      'docs\EUGeneDocumentation.doc',nil,nil, SW_SHOWNORMAL);
end;

procedure TFrame.Website1Click(Sender: TObject);
begin
    ShellExecute(Handle, 'open', 'http://www.eugenesoftware.org',nil,nil, SW_SHOWNORMAL);
end;

procedure TFrame.UserDataSetProceduresMenuItemClick(Sender: TObject);
begin
    ShellExecute(Handle, 'open', 'docs\EUGene user dataset procedures.doc',nil,nil, SW_SHOWNORMAL);
end;

procedure TFrame.SplitMIDsOvertimeProcedureClick(Sender: TObject);
...
procedure TFrame.DyadicMIDData30MenuItemClick(Sender: TObject);
var DyadicMID30 : TDyadic_dispute_data_obj_integrated_format30;
    outfilename : TFileName;
begin
    DyadicGen.FormCreate(Sender);
    DyadicGen.ShowModal;
...
procedure TFrame.HypotheticalAllianceData1Click(Sender: TObject);

begin
   {...}
    AllianceOutputForm.Init(configuration, Sender);
    AllianceOutputForm.ShowModal;
end;

procedure TFrame.DatasetBrowserMenuClick(Sender: TObject);
begin
    Application.CreateForm(TDatasetBrowserForm, DatasetBrowserForm);
...
procedure TFrame.Onset1Click(Sender: TObject);
begin
   try
      user_selections.disputes_selected := all_disputes;
      output_sub_call (ccode_index, nation_list, contiguity_data, configuration, user_selections,
                       output_nondirected_dispute_initiation_dyads);
   except
      on EInOutError do
         begin
         {do nothing but clear the exception, ready for user to try again}
...
procedure TFrame.Year1Click(Sender: TObject);
begin
   try
...
procedure TFrame.OutputEUProjectedJoiningInfo1Click(Sender: TObject);
