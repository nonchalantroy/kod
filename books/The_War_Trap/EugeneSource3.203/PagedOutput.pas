...
procedure TOutput_Options.YearEditExit(Sender: TObject);
begin
   Specified_YrsClick(self);
end;
...
procedure TOutput_Options.SetUserVarTabs(var user_selections: user_selection_type; var configuration: configuration_type);
   ...
   var
      ...
      original_complement_user_var_num: Integer;
      firstboxaddedforthisdataset, lastboxaddedforthisdataset : integer;
      found_complement_box : boolean;
  complementname: string;
  numboxesaddedforthisdataset: Integer;
  some_error_in_complement_box: Boolean;
  last_error_var: Integer;

   function FindBoxNumber(lookingfor_varnumber, startbox, endbox : integer; var wasfound : boolean) : integer;
      {finds check box between start end end with this var number.}
      var currentbox : integer;
      begin
         result := 0;
         wasfound := false;
         currentbox := startbox;
         repeat
            if VarFormCheckBoxes[currentbox].original_user_var_num=lookingfor_varnumber then
               begin
                  wasfound := true;
                  result := currentbox;
               end;
            inc(currentbox);
         until (wasfound) or (currentbox > endbox);
      end;
   ...
   begin
         ...
         for tabsheet := 0 to (num_user_data_sets - 1) do
            begin
               ...
               firstboxaddedforthisdataset := vars_added;         {these counts will be updated along with cumualtive vars_added counter, but reset for each data set}
               numboxesaddedforthisdataset := 0;
               lastboxaddedforthisdataset := 0;
               ...
               if (numsubtabs = 1) then
                     ...
                     for varloop := 0 to (varcounter - 1) do  {add var boxes to the existing panel}
                        begin
                           ...
                           begin
                              ...
                              VarFormCheckBoxes[vars_added].complement_user_var_num := initialized_value;
                              VarFormCheckBoxes[vars_added].complement_check_box := initialized_value;
                              ...
                              inc(vars_added);
                              inc(numboxesaddedforthisdataset);
                           end;
                        end;        {for varloop}
                  ...
                  end; {else if more than one subtab, create multiple subtabs}
               lastboxaddedforthisdataset := firstboxaddedforthisdataset + numboxesaddedforthisdataset - 1;

                  {All variables have now been added to this tab/subtab, which is all the variables
                   for this user data set.}
                  {Now that we've added the variables, need to go back and identify the complement boxes and numbers.}
               some_error_in_complement_box := false;
               last_error_var := 0;
               for varloop := firstboxaddedforthisdataset to lastboxaddedforthisdataset do  {This goes through all vars added for this dat set}
                  begin
                     {These variables all had boxes added, so check them for complement.}
                     complementname := configuration.User_data_set_info.get_data_set_var_reversed_var(VarFormCheckBoxes[varloop].original_user_dataset_num, VarFormCheckBoxes[varloop].original_user_var_num);
                     if (complementname = 'none') then
                        begin
                           {Nothing to search for here, there is no complementary variable}
                        end
                     else  {find complementary variable and save that info.}
                        begin
                           original_complement_user_var_num :=
                              configuration.User_data_set_info.get_data_set_var_number(VarFormCheckBoxes[varloop].original_user_dataset_num, complementname);
                           VarFormCheckBoxes[varloop].complement_user_var_num := original_complement_user_var_num;
                           VarFormCheckBoxes[varloop].complement_check_box := FindBoxNumber(original_complement_user_var_num, firstboxaddedforthisdataset, lastboxaddedforthisdataset, found_complement_box);
                           if not(found_complement_box) then   {Did this code to report an error message only 1x}
                              begin
                                 some_error_in_complement_box := true;
                                 last_error_var := varloop;
                              end;
                        end;    {if have complement}
                  end;   {for varloop first to last}
               if some_error_in_complement_box then
                  EUGeneError('Programming error in setting complementary boxes in Paged Output, for input file.  Notify programmer.  Error for data set '
                     + inttostr(VarFormCheckBoxes[last_error_var].original_user_dataset_num)+', "'
                     + configuration.User_data_set_info.get_data_set_full_name(VarFormCheckBoxes[last_error_var].original_user_dataset_num)
                     + '", varnumber '+inttostr(VarFormCheckBoxes[last_error_var].original_user_var_num)
                     + '.  Errors are likely if this data set is used',
                     1, continue, error_log);

            end;  {for tabsheet, which means same as for each user data set.}
...
var varbox, x, spot_to_add: integer;
    tempmessage : string;
    complement_check_box_number: Integer;
    must_include_complement : boolean;
    original_user_var_num_for_complement: integer;
   ...
   function VarIsOnUserVariableListAlready (avar : Integer; alist : TIntegerArray) : boolean;
      {Figure out if this user variable # is already on the list or not.}
      var x : integer;
         begin
            result := false;
            if (avar <> initialized_value) and (length(alist)>0) then
               {if it's not a real var #, or a real list, then don't bother searching.}
               begin
                  x := 0;
                  repeat
                     if alist[x]=avar then
                        result := true;
                     inc(x);
                  until (result=true) or (x > high(alist));
               end;
         end;

   procedure add_var (avarnum : integer; var avarlist: TIntegerarray);
      {add this variable (avarnum) into the list so it is in order.}
      var movespot: Integer;
          add_at : integer;
      begin
         if length(avarlist)=0 then
            begin
               setlength (avarlist, length(avarlist)+1);
               avarlist[0] := avarnum;
            end
         else
            begin
               {figure out where it should be;  move the others down, and put it in.}
               add_at := -1;
               repeat
                  inc(add_at);
               until (avarlist[add_at] > avarnum) or (add_at>=high(avarlist));
               if (add_at=high(avarlist)) and (avarnum>avarlist[add_at]) then
                  {Here, need to just add at end of list}
                  begin
                     setlength (avarlist, length(avarlist)+1);
                     avarlist[high(avarlist)] := avarnum;
                  end
               else    {adding somewhere in the middle;  move later ones over, then add.}
                  begin
                     setlength (avarlist, length(avarlist)+1);
                     for movespot := high(avarlist)-1 downto add_at do
                       avarlist[movespot+1] := avarlist[movespot];
                     avarlist[add_at] := avarnum;
                  end;
            end;
      end;   {procedure}
  ...
  {In order to preserve settings on vars, do  not blank out the set of variables,
   ...
   for varbox := 0 to high (VarFormCheckBoxes) do
      begin
         ...
         {add this var IF NOT ALREADY ON LIST}
         begin
            if not(VarIsOnUserVariableListAlready (VarFormCheckBoxes[varbox].original_user_var_num,
                                              user_selections.user_data_sets_selections[VarFormCheckBoxes[varbox].original_user_dataset_num].data_set_variables) )
               then   {not already on list, so add it, then check for complement}
               begin
                   {add the var into the appropriate (ordered) spot on list}
                  add_var (VarFormCheckBoxes[varbox].original_user_var_num, user_selections.user_data_sets_selections[VarFormCheckBoxes[varbox].original_user_dataset_num].data_set_variables);
                  include (user_selections.output_format.variables, UserVariables);
                  {Also need to include opposite direction if needed.  There will be a few
                   times this has to happen:
                   data set unit nondirected, variable monadic, output data set directed
                   data set unit nondirected, variable ordered dyadic, output data set directed
                   data set unit nondirected, variable monadic, output data set nondirected
                   data set unit nondirected, variable ordered dyadic, output data set nondirected
                   }
                  {Thought about changing code to see if the complement var was already on there, then
                   checking if needed it.  But since most variables won't have a complement #,
                   didn't do it that way.}
                  must_include_complement := false;
                  if ( ( (configuration.User_data_set_info.get_data_set_unit(VarFormCheckBoxes[varbox].original_user_dataset_num)=nondirected_dyad_year)
                         and (configuration.User_data_set_info.get_data_set_var_unit(VarFormCheckBoxes[varbox].original_user_dataset_num, VarFormCheckBoxes[varbox].original_user_var_num)=monadic)
                         and ( ( (user_selections.output_this = output_directed_dyads)
                                 or (user_selections.output_this = output_directed_dispute_initiation_dyads) ))
                       )
                      or ( (configuration.User_data_set_info.get_data_set_unit(VarFormCheckBoxes[varbox].original_user_dataset_num)=nondirected_dyad_year)
                         and (configuration.User_data_set_info.get_data_set_var_unit(VarFormCheckBoxes[varbox].original_user_dataset_num, VarFormCheckBoxes[varbox].original_user_var_num)=dyadic_ordered)
                         and ( ( (user_selections.output_this = output_directed_dyads)
                                 or (user_selections.output_this = output_directed_dispute_initiation_dyads) ))
                       )
                      or ( (configuration.User_data_set_info.get_data_set_unit(VarFormCheckBoxes[varbox].original_user_dataset_num)=nondirected_dyad_year)
                         and (configuration.User_data_set_info.get_data_set_var_unit(VarFormCheckBoxes[varbox].original_user_dataset_num, VarFormCheckBoxes[varbox].original_user_var_num)=monadic)
                         and ( ( (user_selections.output_this = output_nondirected_dyads)
                                 or (user_selections.output_this = output_nondirected_dispute_dyads) ))
                       )
                      or ( (configuration.User_data_set_info.get_data_set_unit(VarFormCheckBoxes[varbox].original_user_dataset_num)=nondirected_dyad_year)
                         and (configuration.User_data_set_info.get_data_set_var_unit(VarFormCheckBoxes[varbox].original_user_dataset_num, VarFormCheckBoxes[varbox].original_user_var_num)=dyadic_ordered)
                         and ( ( (user_selections.output_this = output_nondirected_dyads)
                  ...
                  must_include_complement := true;

                    { code fragments for creating this command:
                     dataset_unit_of_analysis_type = (no_unit_dataset,
                     country_year, directed_dyad_year, nondirected_dyad_year, annual_data);
                     ...
                     ( (user_selections.output_this = output_directed_dyads)
                     or (user_selections.output_this = output_directed_dispute_initiation_dyads) )

                     (user_selections.output_this = output_nondirected_dyads) or
                     (user_selections.output_this = output_nondirected_dispute_dyads) then

                     user_data_sets_selections[VarFormCheckBoxes[varbox].original_user_dataset_num]
                     user_selections.user_data_sets_selections[VarFormCheckBoxes[varbox].original_user_dataset_num].data_set_variables

                     configuration.User_data_set_info.user_data_set_info[VarFormCheckBoxes[varbox].original_user_dataset_num].data_set_unit
                     configuration.User_data_set_info.user_data_set_info[VarFormCheckBoxes[varbox].original_user_dataset_num].var_info[VarFormCheckBoxes[varbox].original_user_var_num].var_unit

                     configuration.User_data_set_info.user_data_set_info[].var_info: array of one_var_data_type $B6DEAC : $B46938  }

                  if (must_include_complement) then
                     begin
                        {first, does it have a complement?  Then, if it does, is it already checked?}
                        complement_check_box_number := VarFormCheckBoxes[varbox].complement_check_box;
                        if complement_check_box_number <> initialized_value then
                           if not(VarFormCheckBoxes[complement_check_box_number].ACheckBox.checked) then
                              begin
                                 original_user_var_num_for_complement :=
                                    configuration.User_data_set_info.get_data_set_var_number(VarFormCheckBoxes[varbox].original_user_dataset_num, configuration.User_data_set_info.get_data_set_var_reversed_var(VarFormCheckBoxes[varbox].original_user_dataset_num, VarFormCheckBoxes[varbox].original_user_var_num));
                                 if original_user_var_num_for_complement <> VarFormCheckBoxes[varbox].complement_user_var_num then
                                    EUGeneError('Complement numbers did not match in procedure to check new var box in paged output.  Program error; program may fail.  Notify programmer.',0,continue,error_log);
                                 VarFormCheckBoxes[complement_check_box_number].ACheckBox.checked := true;
                                 {After checking box, if the var is not on the list, then add it.}
                                 if not(VarIsOnUserVariableListAlready(VarFormCheckBoxes[complement_check_box_number].original_user_var_num,
                                    user_selections.user_data_sets_selections[VarFormCheckBoxes[complement_check_box_number].original_user_dataset_num].data_set_variables) )
                                    then   {not already on list, so add it}
                                    begin
                                       add_var (VarFormCheckBoxes[complement_check_box_number].original_user_var_num, user_selections.user_data_sets_selections[VarFormCheckBoxes[varbox].original_user_dataset_num].data_set_variables);
                                       {setlength(user_selections.user_data_sets_selections[VarFormCheckBoxes[complement_check_box_number].original_user_dataset_num].data_set_variables,
                                                 length(user_selections.user_data_sets_selections[VarFormCheckBoxes[complement_check_box_number].original_user_dataset_num].data_set_variables)+1);
                                       spot_to_add :=high(user_selections.user_data_sets_selections[VarFormCheckBoxes[complement_check_box_number].original_user_dataset_num].data_set_variables);
                                       user_selections.user_data_sets_selections[VarFormCheckBoxes[complement_check_box_number].original_user_dataset_num].data_set_variables[spot_to_add] :=
                                           VarFormCheckBoxes[complement_check_box_number].original_user_var_num;  }
                                    end;       {newVar is not on list already}

                              end;

                     end;    {must include complement}

               end;
         end;   {if enabled and checked.}
      end; {for varbox}
