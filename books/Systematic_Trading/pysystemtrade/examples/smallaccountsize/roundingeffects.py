...
for accref in [1, 3, 5, 0, 2, 4]: ## 0 is system1_rounded and so on
    ...
    for (instridx, instrument_code) in enumerate(all_instruments):
        allresults_gross.append(all_accounts[instridx][targetref][accref].gross.weekly.ann_mean())
        ...
        allstacknet.append(list(all_accounts[instridx][targetref][accref].net.weekly.iloc[:,0].values))
        allstackcosts.append(list(all_accounts[instridx][targetref][accref].costs.weekly.iloc[:,0].values))
        allstackgross.append(list(all_accounts[instridx][targetref][accref].gross.weekly.iloc[:,0].values))
    ...
    allstackresults['SR'].append(np.nanmean(allstacknet)*(52**.5)/np.nanstd(allstacknet))
