...
def asset_class_count(portfolio, all_assets, assetclasess):
    ...
    assets_in_portfolio=[instr_asset_class(code, assetclasses) for code in portfolio]
    ans=dict([(asset_class, assets_in_portfolio.count(asset_class)) for asset_class in all_assets])
    
    return ans

def which_asset_classes_next(my_portfolio, suitable_instruments, all_assets, assetclasses):
    ## returns a list of asset classes we wish to stock up on
    ## will be those for which my_portfolio is short and suitable_instruments have to spare
    if len(my_portfolio)==0:
        return all_assets
    
    assets_in_my_portfolio=asset_class_count(my_portfolio, all_assets, assetclasses)
    suitable_assets=asset_class_count(suitable_instruments, all_assets, assetclasses)

    available=[asset_class for asset_class in all_assets 
                 if suitable_assets[asset_class]>0]
    
    largest_asset_class_size_in_portfolio=max([assets_in_my_portfolio[asset_class] for asset_class in available])
...
def rank_within_asset_class(asset_class, suitable_instruments, assetclasses, max_positions):
    ## returns the largest of a list of instrument codes, ordered by maximum position size
    instruments_to_check=[code for code in suitable_instruments 
                          if instr_asset_class(code, assetclasses)==asset_class]
    
    instrument_max_positions=[max_positions[code] for code in instruments_to_check]
...
def choose_best_instrument(suitable_asset_classes,  suitable_instruments, assetclasses, max_positions):
    ...
    instrument_max_positions=[max_positions[code] for code in best_max_by_class]
    order=np.array(instrument_max_positions).argsort()
    highest_max_idx=order[-1]
...
def average_correlation(instrument_code,  corrmat, max_positions, instruments, portfolio=[]):
    ...
    sub_corrmat=corrmat[:,[portfolio_index]][instruments.index(instrument_code),:]
