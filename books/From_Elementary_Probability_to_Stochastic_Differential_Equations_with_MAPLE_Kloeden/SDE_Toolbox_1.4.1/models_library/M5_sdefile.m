...
function [out1,out2,out3] = M5_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)

% SDE model definition: drift, diffusion, derivatives and initial conditions.
%
% [out1,out2,out3] = M5_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)
%
...
if nargin < 3 || isempty(flag)
  ...
  x = xsplitted{1}; % e.g. for a thre-dimensional SDE write X1 = xsplitted{1}; X2 = xsplitted{2}; X3 = xsplitted{3}; 
   ...
   case 'ITO'
       driftX = a * x + c;     % the Ito SDE drift
       diffusionX = b * x + d;      % the Ito SDE diffusion
       derivativeX = b;         % the diffusion derivative w.r.t. x
   case 'STRAT'
       driftX = (a - 1/2 * b) * x + c - 1/2 * b * d;     % the Stratonovich SDE drift
       diffusionX = b * x + d;      % the Stratonovich SDE diffusion
       derivativeX = b;         % the diffusion derivative w.r.t. x
   end
