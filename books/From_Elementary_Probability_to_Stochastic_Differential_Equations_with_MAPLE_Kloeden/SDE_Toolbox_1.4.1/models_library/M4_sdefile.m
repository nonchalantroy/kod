...
function [out1,out2,out3] = M4_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)
...
if nargin < 3 || isempty(flag)
  ...
  x = xsplitted{1}; % e.g. for a thre-dimensional SDE write X1 = xsplitted{1}; X2 = xsplitted{2}; X3 = xsplitted{3}; 
   ...
   case 'ITO'
       ...
       diffusionX = b * x;      % the Ito SDE diffusion
       derivativeX = b;         % the diffusion derivative w.r.t. x
   case 'STRAT'
       driftX = a * x - 1/2 * b^2 * x;     % the Stratonovich SDE drift
       diffusionX = b * x;      % the Stratonovich SDE diffusion
       derivativeX = b;         % the diffusion derivative w.r.t. x
   end
