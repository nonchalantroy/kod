...
%
% [out1,out2,out3] = M6_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)
%
...
if nargin < 3 || isempty(flag)
  ...
  x = xsplitted{1}; % e.g. for a thre-dimensional SDE write X1 = xsplitted{1}; X2 = xsplitted{2}; X3 = xsplitted{3}; 
   ...
   case 'ITO'
       driftX = 1/2 * a * (a - 1) * x .^ (1-2/a);     % the Ito SDE drift
       diffusionX = a * x .^ (1-1/a);      % the Ito SDE diffusion
       derivativeX = (a - 1) * x .^ (-1/a);         % the diffusion derivative w.r.t. x
   case 'STRAT'
       driftX = 0;     % the Stratonovich SDE drift
       diffusionX = a * x .^ (1-1/a);      % the Stratonovich SDE diffusion
       derivativeX = (a - 1) * x .^ (-1/a);         % the diffusion derivative w.r.t. x
   end
