...
function [out1,out2,out3] = M10_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)

% SDE model definition: drift, diffusion, derivatives and initial conditions.
%
% [out1,out2,out3] = M10_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)
%
...
if nargin < 3 || isempty(flag)
  ...
  x = xsplitted{1}; % e.g. for a three-dimensional SDE write X1 = xsplitted{1}; X2 = xsplitted{2}; X3 = xsplitted{3};
  
   switch upper(SDETYPE)
   case 'ITO'
       driftX = a * (b-x);           % the Ito SDE drift
       diffusionX = sigma * sqrt(x);      % the Ito SDE diffusion
       derivativeX = sigma/2 * x.^(-1/2);         % the diffusion derivative w.r.t. x
   case 'STRAT'
       driftX = a * (b-x) -1/4*sigma^2;           % the Stratonovich SDE drift
       diffusionX = sigma * sqrt(x);      % the Stratonovich SDE diffusion
       derivativeX = sigma/2 * x.^(-1/2);         % the diffusion derivative w.r.t. x
   end
