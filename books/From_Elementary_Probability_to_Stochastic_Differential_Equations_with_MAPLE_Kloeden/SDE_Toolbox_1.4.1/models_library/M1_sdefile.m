...
function [out1,out2,out3] = M1_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)

% SDE model definition: drift, diffusion, derivatives and initial conditions.
%
% [out1,out2,out3] = M1_sdefile(t,x,flag,bigtheta,SDETYPE,NUMDEPVARS,NUMSIM)
%
...
if nargin < 3 || isempty(flag)
  ...
  x = xsplitted{1}; % e.g. for a three-dimensional SDE write X1 = xsplitted{1}; X2 = xsplitted{2}; X3 = xsplitted{3};
   ...
   case 'ITO'
       driftX = -a*x;           % the Ito SDE drift
       diffusionX = sigma;      % the Ito SDE diffusion
       derivativeX = 0;         % the diffusion derivative w.r.t. x
   case 'STRAT'
       driftX = -a*x;           % the Stratonovich SDE drift
       diffusionX = sigma;      % the Stratonovich SDE diffusion
       derivativeX = 0;         % the diffusion derivative w.r.t. x
   end
