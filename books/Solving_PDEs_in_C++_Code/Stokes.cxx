...
template<class T> class row : public connectedList<rowElement<T> >{
  ...
  template<class T>
  void row<T>::dropPositiveItems(int i, const T&center, double threshold){
    if(next){
      if(((*next)().getColumn() != i)&&
...
template<class T> class sparseMatrix:public list<row<T> >{
 public:
  ...
  sparseMatrix(mesh<triangle>&, sparseMatrix<T>&);
  const sparseMatrix<T> Schur(int,const sparseMatrix<T>&) const;
  ...
  friend const sparseMatrix<T> transposeSquare<T>(const sparseMatrix<T>&);
  ...
  sparseMatrix<T>::Schur(int n, const sparseMatrix<T>&Lambda) const{
    ...
    sparseMatrix<T> A00(n);
    sparseMatrix<T> A01(n);
    sparseMatrix<T> A10(rowNumber()-n);
    for(int i=0; i<rowNumber(); i++){
      if(i<n){
        A00.item[i] = new row<T>(*item[i]);
        A00.item[i]->dropItems(one);
        A01.item[i] = new row<T>(*item[i]);
        A01.item[i]->dropItems(two);
        A01.item[i]->shiftColumns(-n);
      }
      else{
        A10.item[i-n] = new row<T>(*item[i]);
        A10.item[i-n]->dropItems(one);
      }
    }
    return A00 + A01 * Lambda * A10;
  }
  ...
  template<class T>
  sparseMatrix<T>::sparseMatrix(mesh<triangle>&m, sparseMatrix<T>&Lambda){
    int nodes = m.indexing();
    item = new row<T>*[number = 2 * nodes + m.length()];
    for(int i=0; i<number; i++)
      item[i] = 0;
    point gradient[3];
    gradient[0] = point(-1,-1);
    ...
    for(const mesh<triangle>* runner = &m; runner;
      ...
      matrix2 S((*runner)()[1]() - (*runner)()[0](),
	      (*runner)()[2]() - (*runner)()[0]());
      matrix2 Sinverse = inverse(S);
      matrix2 div=(-abs(det(S)/2))*transpose(Sinverse);
      matrix2 weight=abs(det(S)/2)*Sinverse*diffCoef*transpose(Sinverse);
      matrix2 weight2=abs(det(S)/2)*Sinverse*diffCoef2*transpose(Sinverse);
      matrix2 weightMixed =
	  abs(det(S)/2) * Sinverse * mixed * transpose(Sinverse);
      for(int i=0; i<3; i++){
        ...
        if((abs(squaredNorm((*runner)()[i]())-1.)<1.e-7)&&
           ((*runner)()[i]()[0] <= -GammaD)){
	  DirichletBoundary(I) = 0;
	  DirichletBoundary(I+nodes) = 0;
	}
	int I0 = (*runner)().getIndex();
	Lambda.item[I0] = new row<T>(LAMBDA/(abs(det(S))/2.),I0);
	I0 += 2*nodes;
	if(item[I0]){
	  row<T> r((div*gradient[i])[0],I);
	  *item[I0] += r;
	}
	else
	  item[I0] = new row<T>((div*gradient[i])[0],I);
	row<T> r((div*gradient[i])[1],I+nodes);
	*item[I0] += r;
	for(int j=0; j<3; j++){
          int J = (*runner)()[j].getIndex();
	  if(j>=i){
	    if(item[I]){
	      ...
	      *item[I] += r;
	      ...
	      item[I] = new row<T>(gradient[j]*weight*gradient[i],J);
	    if(item[I+nodes]){
	      row<T> r(gradient[j]*weight2*gradient[i],J+nodes);
	      *item[I+nodes] += r;
	    }
	    else
	      item[I+nodes] =
		new row<T>(gradient[j]*weight2*gradient[i],J+nodes);
	  }
	  if(item[I]){
	    row<T> r(gradient[j]*weightMixed*gradient[i],J+nodes);
	    *item[I] += r;
	  }
	  else
	    item[I]=new row<T>(gradient[j]*weightMixed*gradient[i],J+nodes);
	}
      }
    }
    if(Dirichlet)
  ...
  template<class T>
  const sparseMatrix<T>
  transposeSquare(const sparseMatrix<T>&M){
    sparseMatrix<T> Mt(M.rowNumber());
    for(int i=0; i<M.rowNumber(); i++)
...
template<class T> class multigrid{
	...
	void print(const multigrid<T>&mg){
	 ...
	 int count = 0;
	 while((fabs(gamma/gammainit) >= thresholdPCG * thresholdPCG)&&
	   ...
	   count++;
         ...
         printf("PCG=%d; ",count + 1);
       }  //  PCG acceleration

        template<class T>
        const dynamicVector<T>& Stokes(const sparseMatrix<T>&Lambda,const multigrid<T>& MG,
		 const sparseMatrix<T>&A,
                 const dynamicVector<T>&f, dynamicVector<T>&x){

	  dynamicVector<T> r = f - A * x;
	  //r = f;
	  dynamicVector<T> add(r.dim(),0.);
	  for(int i=MG.order();i<A.rowNumber();i++)
	    add(i) = Lambda(i-MG.order(),i-MG.order()) * r[i];
	  add = A * add;
	  dynamicVector<T> fMG(MG.order(),0.);
	  for(int i=0;i<MG.order();i++)
	    fMG(i) = r[i] + add[i];
	  //fMG /= 2 * MU + LAMBDA;
	  dynamicVector<T> xMG(MG.order(),0.);
	  //for(int i=0;i<MG.order();i++)
	  //  xMG(i) = x[i];
	  PCG(MG,MG.a(),fMG,xMG);
	  //MG.Vcycle(fMG,xMG);
	  dynamicVector<T> preRes(A.rowNumber(),0.);
	  for(int i=0;i<MG.order();i++)
	    preRes(i) = xMG[i];
	  dynamicVector<T> ApreRes = A * preRes;
	  for(int i=MG.order();i<A.rowNumber();i++)
	    preRes(i) = Lambda(i-MG.order(),i-MG.order()) * (ApreRes[i]-r[i]);
	    //preRes(i) = LAMBDA * ApreRes[i];
	  /*
	  dynamicVector<double> res=r-A*preRes;
	  for(int i=MG.order();i<A.rowNumber();i++)
	    res(i) += preRes[i]/Lambda(i-MG.order(),i-MG.order());
	  printf("resid=%f\n",sqrt(res*res));
	  double resi=0.;
	  for(int i=0;i<MG.order();i++)
	    resi += res[i]*res[i];
    ...
    template<class T>
    const dynamicVector<T>&
    GMRES(const sparseMatrix<T>&Lambda,const multigrid<T>&MG, const sparseMatrix<T>&A,
          int preIterations, int K,
          const dynamicVector<T>&f, dynamicVector<T>&x){
      for(int i=0;i<preIterations;i++)
        Stokes(Lambda,MG,A,f,x);
      dynamicVector<T> s = x;
      dynamicVector<T> r = x;
      T R[K+1][K+1];
      for(int i=0;i<=K;i++)
      ...
      for(int k=0;k<=K;k++){
        if(k)keep = *Q[k-1];
        Q[k] = k ?
          new dynamicVector<T>(*Q[k-1] - Stokes(Lambda,MG,A,zero,keep))
          : new dynamicVector<T>(Stokes(Lambda,MG,A,f,keep)-x);
        for(int j=0;j<k;j++)
      ...
      return x = s;
        ...
        template<class T>
        void CGS(const sparseMatrix<T>&Lambda,const multigrid<T>& MG,
		 const sparseMatrix<T>&A,
                 const dynamicVector<T>&f, dynamicVector<T>&x){
          const double eps=1.e-15;
	  ...
	  dynamicVector<T> keep(x);
	  dynamicVector<T> rr(Stokes(Lambda,MG,A,f,keep) - x);
	  ...
	  dynamicVector<T> d(zero),v(zero),q(zero);
          ...
          do{
	    keep = pp;
	    v = pp - Stokes(Lambda,MG,A,zero,keep);
	    ...
	    keep = uq;
	    rcgs -= alpha * (uq - Stokes(Lambda,MG,A,zero,keep));
	    ...
	    u = rcgs + beta * q;
	    pp = u + beta * (q + beta * pp);
         } while((omega1/omegainit >= thresholdCGS)&&
	         (++count <= iterationnumber));
  ...
  main(){
    ...
    */
    ...
    for(int i=0; i<adaptiveLevels; i++){
      ...
      f(0) = CoefDelta;
      f(f.dim()/2) = CoefDelta;
      //f(0) = CoefDelta/(2*MU+LAMBDA);
      //f((f.dim()-m.length())/2) = CoefDelta/(2*MU+LAMBDA);
      if(PrintMesh){
	print(m);
	print(A);
        return 0;
      ...
      if(i<adaptiveLevels-1){
        ...
        //NU +=  .099/(adaptiveLevels-1);
        m.refine(x, thresholdAdaptive);
      }
      else{
	int nodes2 = A.rowNumber();
	sparseMatrix<double> Lambda(m.length());
        A = sparseMatrix<double>(m,Lambda);
        A += transposeSquare(A) - diagonal(A);
        multigrid<double> MG(A.Schur(nodes2,Lambda));
	x = dynamicVector<double>(A.rowNumber(),0.);
	dynamicVector<double> fkeep = f;
	f = dynamicVector<double>(A.rowNumber(),0.);
	for(int j=0;j<MG.order();j++)
	  f(j) = fkeep[j];
	//f = dynamicVector<double>(A.rowNumber(),1.);
        //CGS(Lambda,MG,A,f,x);
	for(int j=0;j<10;j++)
          GMRES(Lambda,MG,A,20,10,f,x);
      }
    }
    /*
    sparseMatrix<double> A(m);
